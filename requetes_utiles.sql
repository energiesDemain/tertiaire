...
-- Requete d'extraction du parc en 2010 et 2015 avec energie et systeme de chauffage et besoin 


SELECT cf.COD_BRANCHE, cf.COD_BAT_TYPE, cf.COD_PRODUCTION_CHAUD, cf.COD_ENERGIE, cf.ANNEE, 
sum(cf.SURFACES) AS SURFACES,sum(cf.BESOIN) AS BESOIN, sum(cf.CONSOMMATION_EF) AS CONSOMMATION_EF
FROM
(SELECT  substr(cs.ID,1,2) as COD_BRANCHE, substr(cs.ID,5,2) as COD_BAT_TYPE, 
substr(cs.ID,13,2) as COD_PRODUCTION_CHAUD, substr(cs.ID,17,2) as COD_ENERGIE, 
cs.ANNEE, cs.SURFACES, cs.BESOIN, cs.CONSOMMATION_EF 
FROM 
(SELECT ck.ID, ck.ANNEE, ck.CONSOMMATION_EF, cj.BESOIN, ci.SURFACES
FROM CONSO_RT_RESULTATS ck, BESOIN_RT_RESULTATS cj , PARC_RESULTATS ci 
WHERE ck.ID = cj.ID AND ck.ID = ci.ID AND ck.USAGE = 'Chauffage' AND
(ck.ANNEE = '2015') AND
 cj.USAGE = 'Chauffage' AND
(cj.ANNEE = '2015') AND
(ci.ANNEE = '2015')) cs) cf
GROUP BY  cf.COD_BRANCHE, cf.COD_BAT_TYPE, cf.COD_PRODUCTION_CHAUD,  cf.COD_ENERGIE, cf.ANNEE
