...
wbfiles[2] =   "../tertiaire_to_export/resultats_AME_run2/2050_sanspol_sansCC_newcalib/Results_csv"
wbfiles[3] = "../tertiaire_to_export/resultats_AME_run2/2050_allpol_sansCEE_CC_newcalib/Results_csv"
wbfiles[4] = "../tertiaire_to_export/resultats_AME_run2/2050_allpol_CEE_CC_newcalib/Results_csv"
wbfiles[5] = "../tertiaire_to_export/resultats_AME_run2/2050_allpol_CEE_CC/table_resultat_2050_allpol_CEE_CC.xlsx"
wbfiles[6] = "../tertiaire/model/Result_csv/2050_allpol_CEE_sansCC"
#### noms des scenarios ###

wbname = c( "S0 : AME 2016",
            "S1 : AME 2017 run2 sans pol",
            "S2 : AME 2017 run2 sans CEE",
            "S3 : AME 2017 run2 CEE",
            "S4 : AME 2017 run2 Enerdata",
            "S5 : AME 2017 run2 sans CC" 
)


#### choix du scenario de référence 
scenario_ref = c("S1 : AME 2016")
scenario_ref = c( "S2 : AME 2017 run2 sans CEE")
scenario_toexport = c( "S3 : AME 2017 run2 CEE")
wbdir_scenario_toexport = c("../tertiaire_to_export/resultats_AME_run2/2050_allpol_CEE_CC_newcalib/")
...
* S0 AME 2016-2017 reconstitué à partir des paramètres et des hypothèses précédentes, simulation sur l'ancienne version du modèle
* S1 AME 2017 run2 sans pol : version recalibrée du modèle, sans mesures AME, sans CCE
* S2 AME 2017 run2 all pol sans CEE : version recalibrée du modèle et ajouts des mesures AME 2017 sans les CEE
* S3 : AME 2017 run2 CEE : idem + CEE modélisé à l'aide d'un signal prix de 2015 à 2020
* S4 : AME 2017 run2 Enerdata : version précédente envoyée à Enerdata
* S5 : AME 2017 run2 sans CC : run2 CEE sans adaptation au CC
...
```{r comp_parc_CEREN, fig.width=20, fig.height=9, echo=F, warning=F,fig.cap='Comparaison avec le parc CEREN par branche', dev=c('png', 'pdf'),  fig.show='hold'}
...
```{r comp_parc_CEREN_ener, fig.width=20, fig.height=9, echo=F, warning=F,fig.cap='Comparaison avec le parc CEREN par énergie de chauffage', dev=c('png', 'pdf')}
...
ggplot(PM_syst_neuf) +
           ...
           facet_wrap(~scenario, nrow = 1) +
           mytheme_facet_plot  +
    scale_x_discrete(breaks = seq(2011,2051,5), labels =
                    seq(2010,2050,5))  +
...
```{r Surfaces_Renov, fig.width=20, fig.height=13, echo=F, warning=F, fig.show='hold', dev=c('png', 'pdf')}
...
ggplot(Surfaces_renov_gestes_cum) + 
  ...
  scale_x_discrete(breaks = seq(2010,2050,5), labels =
                    seq(2010,2050,5)) +
  scale_y_continuous(limits  = c(0,150)) + facet_wrap(~scenario) + 
  scale_fill_brewer(palette = fillpalette) 


ggplot(Surfaces_renov_syst_cum) + 
  geom_ribbon(aes(ymin = 0, annee, ymax =  surface_cum_syst , group =SYSTEME_CHAUD, fill = SYSTEME_CHAUD)) +  ylab("Millions de m²") + ggtitle("Surfaces rénovées par système de chauffage") + xlab("annee") +
  mytheme_facet_plot +
