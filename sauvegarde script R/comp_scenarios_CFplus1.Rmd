...
wbfiles  = "../tertiaire/model/Result_csv/publi_thema/prix_AME_noTC_verif"
wbfiles[2]  = "../tertiaire/model/Result_csv/publi_thema/prix_AME_TC600"
wbfiles[3]  = "../tertiaire/model/Result_csv/publi_thema/prix_AME_TC600allenergies"
wbfiles[4]  = "../tertiaire/model/Result_csv/publi_thema/prix_AME_CEE_AMS"
wbfiles[5]  = "../tertiaire/model/Result_csv/publi_thema/prix_AME_OR_AMS"
wbfiles[6]  = "../tertiaire/model/Result_csv/publi_thema/2050_AMS_final"
...
wbname = c("S0 Nopol","S1 TC600","S1b TC600bis","S2 CEE","S3 OR","S4 AMS",  paste0("S",5:(length(wbfiles))))

### choix du scenario de référence 

scenario_ref =c("S0 Nopol")
scenario_toexport = c("S4 AMS")
scenario_sansCEE = c( "S0 Nopol")
scenario_comp_AMS = c("S0 Nopol","S1 TC600","S1b TC600bis","S2 CEE","S3 OR","S4 AMS")

wbdir_scenario_toexport = c("../publication_thema/run_AMS/")
write_excel =T

scenarioshortnames = substring(wbname,1,2)
scenarioshortnames = c("S0 Nopol","S1 TC600","S1b TC600bis","S2 CEE","S3 OR","S4 AMS", paste0("S",5:(length(wbfiles))))
scenarioshortnames = wbname


scenariolabels = c("S0 Nopol" = "S0", "S1 TC600" = "TC600", "S1b TC600bis" = "TC600bis",
                  "S2 CEE"= "CEE","S3 OR" = "OR","S4 AMS"="AMS")
...
# FICHE RESULTATS politiques unitaires
...
ggsave("../publication_thema/présentation/Conso_2050_polunitaire.png",width = 14, height=12)
...
ggplot(INV_regl_bilan_cum) + geom_bar(aes(scenario,investissement/10^3,fill=reglementation),stat = "identity",position = "stack")+mytheme_facet_plot + scale_y_continuous(breaks = seq(0,150,5))+
...
Em_nationale_energie_type_usage[scenario =="S4 AMS",EmMtCO2_socio := conso_tWhEF*FE_AMS*10^9*10^-12]
Em_nationale_energie_type_usage[scenario !="S4 AMS",EmMtCO2_socio := conso_tWhEF*FE_AME*10^9*10^-12]
Em_nationale_energie_usage[scenario =="S4 AMS",EmMtCO2_socio := conso_tWhEF*FE_AMS*10^9*10^-12]
Em_nationale_energie_usage[scenario !="S4 AMS",EmMtCO2_socio := conso_tWhEF*FE_AME*10^9*10^-12]
...
ggsave("../publication_thema/présentation/Em_tot_polunitaire.png",width = 18, height=12)
...
ggsave("../publication_thema/présentation/Em_chauffage_polunitaire.png",width = 18, height=12)
...
```{r,  echo=F, warning=F,include = F}
#### impact politique par politique
Emcumtot = Em_nationale
Emcumtot[,sum(EmMtCO2), by="scenario"]

Consocumtot = Conso_nationale_usage
Consocumtot[,sum(conso_tWhEF), by="scenario"]
...
# 
prix_all[annee %in% c(2015, 2030,2050)]
...
#### attendtion pour AME on prend ici sans taxe car tous les scénarios n'ont pas de TC
bilan_socio[,Depense_usager_AME := conso_tWhEF*prix_socio_eco_AME*10^-3]
...
#### recettes taxe carbone 

bilan_socio[energie %in% c("Electricité", "Urbain","Autres") & scenario != "S1b TC600bis", TC_AMS := 0]

bilan_socio[,Recettes_TC_AME := 0]
bilan_socio[scenario == "S1 TC600",Recettes_TC_AMS := Emiss_Mt_AME*TC_AMS*10^-3]
bilan_socio[scenario == "S1b TC600bis",Recettes_TC_AMS := Emiss_Mt_AME*TC_AMS*10^-3]
bilan_socio[scenario == "S4 AMS",Recettes_TC_AMS := Emiss_Mt_AMS*TC_AMS*10^-3]


bilan_socio[scenario%in% c("S1 TC600","S1b TC600bis","S4 AMS") == F,Recettes_TC_AMS :=0]

bilan_socio[,sum(Recettes_TC_AMS), by="scenario"]
bilan_socio[,sum(Recettes_TC_AME), by="scenario"]
...
bilan_socio[,sum(Recettes_taxes_ener_AME), by="scenario"]
bilan_socio[,sum(Recettes_taxes_ener_AMS), by="scenario"]


#### subvention élec 
bilan_socio[,sub_elec := 0]

bilan_socio[scenario=="S4 AMS" & energie == "Electricité",sub_elec := conso_tWhEF*Taxe_elec*10^-3]

bilan_socio[,sum(sub_elec), by="scenario"]


##### ajoute les taxes ener pour l'usager
bilan_socio[,sum(Depense_usager_AME), by="scenario"]


bilan_socio[scenario!="S4 AMS",
            Depense_usager_AME := Depense_usager_AME + Recettes_taxes_ener_AME]

##### ajoute la composante  aux scénarios avec TC 
bilan_socio[scenario!="S4 AMS",
            Depense_usager_AME := Depense_usager_AME  + Recettes_TC_AMS]

##### ajoute les taxes ener pour l'usager
bilan_socio[,sum(Depense_usager_AMS), by="scenario"]


test =  bilan_socio[scenario %in% c("S1 TC600","S1b TC600bis", "S4 AMS")]

test[,list(sum(Recettes_TC_AMS),sum(Emiss_Mt_AME), sum(conso_tWhEF), sum(Emiss_Mt_AME)/sum(conso_tWhEF)),  by=c("scenario", "energie")]
test[,list(sum(Recettes_TC_AMS),sum(Emiss_Mt_AMS), sum(conso_tWhEF), sum(Emiss_Mt_AMS)/sum(conso_tWhEF)),  by=c("scenario", "energie")]

test[,list(sum(Recettes_TC_AMS),sum(Emiss_Mt_AME), sum(conso_tWhEF), sum(Emiss_Mt_AME)/sum(conso_tWhEF)),  by=c("scenario")]
...
test[energie == "Gaz", list(sum(Recettes_TC_AME),sum(Emiss_Mt_AME), 
                                    sum(Recettes_TC_AMS),sum(Emiss_Mt_AMS), sum(conso_tWhEF)),by=c("scenario")]
test[,sum(Recettes_TC_AMS), by="scenario"]
...
table_results_socio = data.table(scenario = c("Nopol","TC600","TC600bis","CEE","OR","AMS")) 

## facture
table_results_socio[scenario!= "AMS",Fact :=bilan_socio_cum2[scenario != "S4 AMS",Depense_socio_AME_actu] ]
table_results_socio[scenario == "AMS",Fact :=bilan_socio_cum2[scenario == "S4 AMS",Depense_socio_AMS_actu] ]

## INV
table_results_socio[scenario!= "AMS",INV :=bilan_socio_cum2[scenario != "S4 AMS", INV_Md_actu] ]
table_results_socio[scenario == "AMS",INV :=bilan_socio_cum2[scenario == "S4 AMS",INV_Md_actu] ]

## EMissions
table_results_socio[scenario!= "AMS",Em :=bilan_socio_cum2[scenario != "S4 AMS",Emiss_Mt_AME_actu] ]
table_results_socio[scenario == "AMS",Em :=bilan_socio_cum2[scenario == "S4 AMS",Emiss_Mt_AMS_actu] ]

table_results_socio[,CoutTot := Fact + INV]

## coût tonne
table_results_socio[,CoutTonne :=(CoutTot - table_results_socio[scenario== "Nopol",CoutTot])/-(Em - table_results_socio[scenario== "Nopol",Em])*10^3, by="scenario"]


#### recettes taxes
table_results_socio[scenario!= "AMS",TC :=bilan_socio_cum2[scenario != "S4 AMS",Recettes_TC_AMS_actu] ]
table_results_socio[scenario == "AMS",TC :=bilan_socio_cum2[scenario == "S4 AMS",Recettes_TC_AMS_actu] ]

table_results_socio[scenario!= "AMS",Taxes_ener :=bilan_socio_cum2[scenario != "S4 AMS",Recettes_taxes_ener_AME_actu] ]
table_results_socio[scenario == "AMS",Taxes_ener :=bilan_socio_cum2[scenario == "S4 AMS",Recettes_taxes_ener_AMS_actu] ]

table_results_socio[scenario!= "AMS",sub_elec :=bilan_socio_cum2[scenario != "S4 AMS",Subvention_elec_actu] ]
...
table_results_socio[,CoutTonne_COFP :=(CoutTot+ COFP - table_results_socio[scenario== "Nopol",CoutTot+ COFP])/-(Em - table_results_socio[scenario== "Nopol",Em])*10^3, by="scenario"]
...
table_results_usager =  data.table(scenario = c("Nopol","TC600","TC600bis","CEE","OR","AMS")) 

## facture
table_results_usager[scenario!= "AMS",Fact :=bilan_usager_cum2[scenario != "S4 AMS",Depense_usager_AME_actu] ]
table_results_usager[scenario == "AMS",Fact :=bilan_usager_cum2[scenario == "S4 AMS",Depense_usager_AMS_actu] ]

## INV
table_results_usager[scenario!= "AMS",INV :=bilan_usager_cum2[scenario != "S4 AMS", INV_Md_actu] ]
table_results_usager[scenario == "AMS",INV :=bilan_usager_cum2[scenario == "S4 AMS",INV_Md_actu] ]

## EMissions
table_results_usager[scenario!= "AMS",Em :=bilan_usager_cum2[scenario != "S4 AMS",Emiss_Mt_AME_actu] ]
table_results_usager[scenario == "AMS",Em :=bilan_usager_cum2[scenario == "S4 AMS",Emiss_Mt_AMS_actu] ]
...
# bilan_usager_cum[, Diff_AMS_AME := get("S4 AMS")-get("S2 AME")]
# 
...
table_results_socio[,scenario2 :=c("Nopol","CC600","CC600elec","CEE","OR","SIM")]
table_results_usager[,scenario2 :=c("Nopol","CC600","CC600elec","CEE","OR","SIM")]

graph_taxes = bilan_socio[scenario %in% c("S1 TC600","S1b TC600bis","S4 AMS")]
graph_taxes[ ,TC := Recettes_TC_AMS]

graph_taxes[scenario %in% c("S4 AMS") == F ,Taxe_ener_graph := Recettes_taxes_ener_AME]
graph_taxes[scenario %in% c("S4 AMS") ,Taxe_ener_graph :=  Recettes_taxes_ener_AMS]
graph_taxes[scenario %in% c("S4 AMS") == F ,coutotal_graph := Depense_usager_AME + INV_Md]
graph_taxes[scenario %in% c("S4 AMS") ,coutotal_graph := Depense_usager_AMS+ INV_Md]
graph_taxes[scenario %in% c("S4 AMS") == F ,facture_graph := Depense_usager_AME]
graph_taxes[scenario %in% c("S4 AMS") ,facture_graph := Depense_usager_AMS]

graph_taxes[,scenario2 :=factor(scenario, levels = c("S1 TC600","S1b TC600bis","S4 AMS"),labels =c("CC600","CC600elec","SIM"))]


write.table(table_results_socio[, list(scenario,Fact,INV,COFP,CoutTot_COFP,Em,CoutTonne_COFP, CoutTot, CoutTonne)],
            file = "../publication_thema/présentation/bilan_socio_polunitaire.csv",quote = T, sep=";",row.names = F)
write.table(table_results_socio[, list(scenario, TC,Taxes_ener,sub_elec,COFP)],
            file = "../publication_thema/présentation/bilan_Etat_polunitaire.csv",quote = T, sep=";",row.names = F)

write.table(table_results_usager[, list(scenario, Fact,INV,CoutTot)],
            file = "../publication_thema/présentation/bilan_usager_polunitaire.csv",quote = T, sep=";",row.names = F)

```

```{r Evol_Recettes_taxes, fig.width=25, fig.height=10, echo=F, warning=F,fig.cap='Evolution recettes de la CC', dev=c('png', 'pdf')}
ggplot() + geom_line(data = graph_taxes, aes(annee,TC*10^3, group = scenario2, color= scenario2), size = 4) + 
  theme(axis.text.y = element_text(size = 30))+ scale_x_discrete(breaks =c(seq(2010,2050,10))) + ylab("Millions d'euros") + 
  mytheme_facet_plot + xlab("") 

ggsave("../publication_thema/présentation/EvolRecettesCCall.png",width = 24, height=12)


ggplot() + geom_line(data = graph_taxes, aes(annee,Taxe_ener_graph, group = scenario, color= scenario)) + mytheme_facet_plot
ggplot() + geom_line(data = graph_taxes, aes(annee,coutotal_graph, group = scenario, color= scenario), size = 4) + 
  theme(axis.text.y = element_text(size = 30))+ scale_x_discrete(breaks =c(seq(2010,2050,10))) + ylab("Millions d'euros") + 
  mytheme_facet_plot + xlab("") 
ggplot() + geom_line(data = graph_taxes, aes(annee,facture_graph, group = scenario, color= scenario), size = 4) + 
  theme(axis.text.y = element_text(size = 30))+ scale_x_discrete(breaks =c(seq(2010,2050,10))) + ylab("Millions d'euros") + 
  mytheme_facet_plot + xlab("") + scale_y_continuous(limits = c(0,8)) 

```


```{r Comp_pol, fig.width=25, fig.height=10, echo=F, warning=F,fig.cap='Evolution recettes de la CC', dev=c('png', 'pdf')}

ggplot(table_results_socio) + geom_point(aes(Em, CoutTonne_COFP, color=scenario),show.legend = F, size = 8) + theme(axis.text.y = element_text(size = 30)) + ylab("Euros par tonne évitée") +   geom_text(aes(Em, CoutTonne_COFP, color=scenario, label = scenario2), vjust=-0.8, show.legend = F, size = 12) + scale_y_continuous(limits = c(-200,1200)) + scale_x_continuous(limits = c(300,450)) +
  geom_vline(aes(xintercept = table_results_socio[scenario == "Nopol",Em]), size = 2) +
  mytheme_facet_plot + xlab("Emissions cumulées (MtCO2)")

ggsave("../publication_thema/présentation/Comppol_couttonne.png",width = 24, height=12)

ggplot(table_results_usager[scenario2 != "Nopol"]) + geom_point(aes(Em, CoutTot, color=scenario),show.legend = F, size = 8) + theme(axis.text.y = element_text(size = 30)) + ylab("Coût usager (Mds euros)") +   geom_text(aes(Em, CoutTot, color=scenario, label = scenario2), vjust=-0.8, show.legend = F, size = 12) +  scale_x_continuous(limits = c(300,450)) +
  mytheme_facet_plot + xlab("Emissions cumulées (MtCO2)") + scale_y_continuous(limits = c(175,250))

ggsave("../publication_thema/présentation/Comppol_Coutusager.png",width = 24, height=12)


ggplot(table_results_socio[scenario2 != "Nopol"]) + geom_point(aes(Em, TC+Taxes_ener-sub_elec, color=scenario),show.legend = F, size = 8) + theme(axis.text.y = element_text(size = 30)) + ylab("Recettes taxes (Mds euros)") +   geom_text(aes(Em,TC+Taxes_ener-sub_elec, color=scenario, label = scenario2), vjust=-0.8, show.legend = F, size =12) + scale_y_continuous(limits = c(-10,50)) + scale_x_continuous(limits = c(300,450)) +
  mytheme_facet_plot + xlab("Emissions cumulées (MtCO2)")

ggsave("../publication_thema/présentation/Comppol_recettes.png",width = 24, height=12)
