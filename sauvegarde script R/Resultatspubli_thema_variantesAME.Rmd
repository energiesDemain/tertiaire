...
wbfiles[2] = "../tertiaire/model/Result_csv/publi_thema/prix_AME_TC_verif"
wbfiles[3]  = "../tertiaire/model/Result_csv/publi_thema/2050_AME_final_verif"
wbfiles[4]  = "../tertiaire/model/Result_csv/publi_thema/2050_AME_final_plusTC600"
wbfiles[5]  = "../tertiaire/model/Result_csv/publi_thema/2050_AME_final_plusCEE"
wbfiles[6]  = "../tertiaire/model/Result_csv/publi_thema/2050_AME_final_plusOR"
wbfiles[7]  = "../tertiaire/model/Result_csv/publi_thema/2050_AMS_final_verif"

####### noms des scenarios ########

wbname = c( paste0("S",1:length(wbfiles)))
  
wbname = c("Nopol","TC100","AME","AMETC600","AMECEE","AMEOR","AMS",paste0("S",8:(length(wbfiles))))
...
scenario_comp_AMS = c("AME","AMETC600","AMECEE","AMEOR","AMS")

wbdir_scenario_toexport = c("../publication_thema/run_AME/")
write_excel =F

scenarioshortnames = substring(wbname,1,2)
scenarioshortnames = c("Nopol","TC100","AME","AMETC600","AMEOR","AMS",paste0("S",8:(length(wbfiles))))
scenarioshortnames = wbname


scenariolabels = c("AME" = "Réf", wbname[2:length(wbname)])
...
##### prix des énergies
require(readxl)
prix_ener_socio = data.table(read_excel("../publication_thema/hypothèses_prix/Prix_scenario_thema.xls", sheet = "prix_socio_eco_AME"))
...
prix_ener_socio = melt(prix_ener_socio, id.vars = "annee",variable.name = "energie",value.name = "prix_socio_eco")
...
prix_all = merge(prix_ener_socio,prix_usager_AME, by=c("annee","energie"))
...
#### Emissions chauffage
bilan_socio = merge(COnso_an_chauffage, FE, by=c("annee","energie", "usage"))

bilan_socio[, Emiss_Mt := conso_tWhEF*FE*10^-3]

#### Depenses_ener 
bilan_socio = merge(bilan_socio,prix_all, by=c("annee","energie"))

bilan_socio[,Depense_socio := conso_tWhEF*prix_socio_eco*10^-3]
...
bilan_socio[,Depense_usager := conso_tWhEF*prix_usager_AMS*10^-3]

### aggreg sur toute les ener
bilan_socio = bilan_socio[,list(conso_tWhEF=sum(conso_tWhEF),Depense_socio = sum(Depense_socio),
                                Depense_usager = sum(Depense_usager),
                                Emiss_Mt = sum(Emiss_Mt)), by=c("scenario","annee")]
...
bilan_socio[, Emiss_Mt_actu:= Emiss_Mt*coefactu]
bilan_socio[,Depense_socio_actu:= Depense_socio*coefactu]

#### ajout 10 ans à la fin
bilan_socio[annee=="2050", conso_tWhEF_actu:= conso_tWhEF*coefactu*10]
bilan_socio[annee=="2050", Emiss_Mt_actu:= Emiss_Mt*coefactu*10]
bilan_socio[annee=="2050",Depense_socio_actu:= Depense_socio*coefactu*10]


bilan_socio_cum = bilan_socio[,list(INV_Md_actu = sum(INV_Md_actu ), INV_Md = sum(INV_Md),
                  conso_tWhEF_actu=sum(conso_tWhEF_actu), conso_tWhEF=sum(conso_tWhEF), 
                  Emiss_Mt_actu=sum(Emiss_Mt_actu), Emiss_Mt=sum(Emiss_Mt),
                  Depense_socio_actu = sum(Depense_socio_actu),Depense_socio=sum(Depense_socio)) ,
                  by="scenario"]

bilan_socio_cum = melt(bilan_socio_cum, id.vars = "scenario")

bilan_socio_cum = dcast.data.table(bilan_socio_cum, variable~scenario)
bilan_socio_cum[, Diff_AME_nopol := AME-Nopol]
bilan_socio_cum[, Diff_TC100_nopol := TC100-Nopol]
bilan_socio_cum[, Diff_AME_AMETC600 := AMETC600-AME]
bilan_socio_cum[, Diff_AME_AMECEE := AMECEE-AME]
bilan_socio_cum[, Diff_AMS_AME := AMS-AME]

bilan_socio_TC100_Nopol = melt(bilan_socio_cum, id.vars = "variable", measure.vars ="Diff_TC100_nopol", variable.name = "Diff")
bilan_socio_TC100_Nopol = dcast.data.table(bilan_socio_TC100_Nopol, Diff~variable)
bilan_socio_TC100_Nopol[,cout_socio_tonne := (INV_Md_actu+Depense_socio_actu)/-Emiss_Mt_actu*10^3]
...
bilan_socio_AME_Nopol[,cout_socio_tonne := (INV_Md_actu+Depense_socio_actu)/-Emiss_Mt_actu*10^3]

bilan_socio_AME_AMETC600 = melt(bilan_socio_cum, id.vars = "variable", measure.vars ="Diff_AME_AMETC600", variable.name = "Diff")
bilan_socio_AME_AMETC600  = dcast.data.table(bilan_socio_AME_AMETC600 , Diff~variable)
bilan_socio_AME_AMETC600[,cout_socio_tonne := (INV_Md_actu+Depense_socio_actu)/-Emiss_Mt_actu*10^3]

bilan_socio_AME_AMECEE = melt(bilan_socio_cum, id.vars = "variable", measure.vars ="Diff_AME_AMECEE", variable.name = "Diff")
bilan_socio_AME_AMECEE  = dcast.data.table(bilan_socio_AME_AMECEE , Diff~variable)
bilan_socio_AME_AMECEE[,cout_socio_tonne := (INV_Md_actu+Depense_socio_actu)/-Emiss_Mt_actu*10^3]
...
bilan_socio_AMS_AME[,cout_socio_tonne := (INV_Md_actu+Depense_socio_actu)/-Emiss_Mt_actu*10^3]

results_socio = rbind(bilan_socio_TC100_Nopol,bilan_socio_AME_Nopol,bilan_socio_AME_AMETC600,bilan_socio_AME_AMECEE,bilan_socio_AMS_AME)
