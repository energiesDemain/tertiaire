...
wbfiles  = "../tertiaire/model/Result_csv/publi_thema/avec_calib_CODAH/2050_nopol_prixconstant_noRT2012"
wbfiles[2]  = "../tertiaire/model/Result_csv/publi_thema/avec_calib_CODAH/2050_nopol_prixconstant"
wbfiles[3]  = "../tertiaire/model/Result_csv/publi_thema/avec_calib_CODAH/2050_nopol_prixcroissants_noRT2012"
wbfiles[4]  = "../tertiaire/model/Result_csv/publi_thema/avec_calib_CODAH/2050_nopol_prixcroissants"
...
scenario_toexport_list = c("S0 Nopol",
                      "S1 AME",
                      "S2 AMS",
                      "S3 AMS1",
                      "S4 AMS1 GJ",
...
CC_to_use = data.table(scenario=wbname,CC=c("CC_AMSGJ","CC_AME", "CC_AMS","CC_AMS","CC_AMS"
                                            #"CC_AMSGJ","CC_AMS","CC_AMSGJ","CC_AMS","CC_AMS"
                                            ))

CC_toutes_ener = data.table(scenario=wbname,CC=c(F,F,F,F,F
                                            ))
#### prix et taxes à utiliser
prix_usager_to_use = data.table(scenario=wbname,prix=c("prix_usager_2018","prix_usager_AME", 
                                                    # "prix_usager_AMS",p
...
tax_to_use = data.table(scenario=wbname,taxe=c("taxes_usager_2018","taxes_usager_AME",
                                            # "taxes_usager_AMS",
...
```{r Evol_prix , fig.width=20, fig.height=14, echo=F, warning=F,fig.cap='Evolution du parc', dev=c('png', 'pdf')}
...
```{r Evol_Em_energie, fig.width=25, fig.height=12, echo=F, warning=F,fig.cap='Evolution des émissions pour tous les usages par énergie', dev=c('png', 'pdf')}
if(check_Em == T) {
ggplot(Em_nationale_energie) +
           ...
           mytheme_facet_plot+ theme(legend.position = "top") +
  scale_x_discrete(breaks = seq(2010,2050,5), labels =
                    seq(2010,2050,5))  + scale_fill_manual("",values=color_energie$color) +  scale_color_manual("",values=color_energie$color)
}
```
...
ggplot(Em_nationale_energie_usage[usage == "Chauffage"]) +
           ...
           mytheme_facet_plot+ theme(legend.position = "top") +
  scale_x_continuous(breaks = seq(2010,2050,10), labels =
                    seq(2010,2050,10)) + scale_fill_manual("",values=color_energie$color)  +  scale_color_manual("",values=color_energie$color) + 
  ylab("MtCO2") +  xlab("Année")+
  theme(axis.text.y = element_text(size = 30),strip.text.x = element_text(size=30), legend.text = element_text(size = 20))
}
```
...
```{r Evol_Em_Chauff, fig.width=25, fig.height=10, echo=F, warning=F,fig.cap='Evolution des émissions totales', dev=c('png', 'pdf')}
if(check_Em == T) {
...
table_results_socio = bilan_socio_cum2[,list( scenario,INV=INV_Md_actu,Fact = Facture_socio_actu, Em=EmMtCO2_actu,Conso=conso_tWhEF_actu, Taxes =Recettes_taxes_ener_actu)]
...
table_results_socio[,COFP := 0.25*(Taxes)]
...
##### bilan Etat
 ...
 Bilan_Etat=bilan_socio_cum2[,list(CC = Recettes_CC_actu, taxes_autres=Recettes_taxes_ener_actu-Recettes_CC_actu, Total =Recettes_taxes_ener_actu )]
...
bilan_usager[, EmMtCO2_actu:= EmMtCO2*coefactu]
...
bilan_usager_cum = bilan_usager[,list(INV_Md_actu = sum(INV_Md_actu ),
                                    INV_Md = sum(INV_Md),
                                    conso_tWhEF_actu=sum(conso_tWhEF_actu),
                                    conso_tWhEF=sum(conso_tWhEF),
                                    EmMtCO2_actu=sum(EmMtCO2_actu),
...
table_results_usager = bilan_usager_cum2[,list( scenario,INV=INV_Md_actu,Fact =  Depense_usager_actu, Em=EmMtCO2_actu,Conso=conso_tWhEF_actu)]
...
```{r, echo=F, warning=F,}
 if(check_bilan_socio==T){
pander(Bilan_Etat, caption = "bilan pour l'Etat")
 }
```

```{r, echo=F, warning=F,}
 if(check_bilan_socio==T){
pander(table_results_usager, caption = "bilan pour les usagers")
 }
```
```{r, echo=F, warning=F,}
 if(check_bilan_socio==T){
pander(table_results_socio, caption = "bilan pour la société")
 }
```
...
ggplot() + geom_line(data = bilan_socio, aes(annee,Recettes_CC, group = scenario, color= scenario), size=2) + mytheme_facet_plot+ 
  scale_color_brewer("", palette=scenario_palette) + ylab("recettes CC")
 }
```
...
ggplot() + geom_line(data = bilan_socio, aes(annee,Recettes_taxes_ener-Recettes_CC, group = scenario, color= scenario), size=2) + mytheme_facet_plot+
   scale_color_brewer("",palette=scenario_palette) + ylab("recettes hors CC")
}
```
...
ggplot() + geom_line(data = bilan_socio, aes(annee,Recettes_taxes_ener, group = scenario, color= scenario), size=2) + mytheme_facet_plot+
   scale_color_brewer("",palette=scenario_palette) + ylab("recettes toutes taxes")
}
