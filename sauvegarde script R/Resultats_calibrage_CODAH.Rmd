...
wbfiles[2]  = "../tertiaire/model/Result_csv/publi_thema/avec_calib_CODAH/2050_AME"
#wbfiles[4]  = "../tertiaire/model/Result_csv/publi_thema/2050_AMS_final"
#wbfiles[4]  = "../tertiaire/model/Result_csv/publi_thema/avec_calib_CODAH/2050_AME_old"
wbfiles[3]  = "../tertiaire/model/Result_csv/publi_thema/avec_calib_CODAH/2050_AMS_nosubENR"
#wbfiles[6]  = "../tertiaire/model/Result_csv/publi_thema/avec_calib_CODAH/2050_AMS1"
wbfiles[4]  = "../tertiaire/model/Result_csv/publi_thema/avec_calib_CODAH/2050_AMS1_biog150"
wbfiles[5]  = "../tertiaire/model/Result_csv/publi_thema/avec_calib_CODAH/2050_AMS2"
wbfiles[6]  = "../tertiaire/model/Result_csv/publi_thema/avec_calib_CODAH/2050_AMS1_gilets_jaunes"
...
wbname = c("S0 Nopol",
           ...
           "S2 AME new", 
           #"S3 AMS",
           "S4 AMS new","S5 AMS1","S6 AMS2","S5 AMS1 GJ","S6 AMS2 GJ", paste0("S",7:(length(wbfiles))))

### choix du scenario de référence 

scenario_ref = c("S2 AME new")
scenario_toexport = c("S2 AME new")
scenario_sansCEE = c( "S0 Nopol")
scenario_comp_AMS = c("S0 Nopol",
                      #"S1 AME",
                      "S2 AME new",
                      #"S3 AMS",
                      "S4 AMS new", "S5 AMS1", "S6 AMS2", "S5 AMS1 GJ","S6 AMS2 GJ")

wbdir_scenario_toexport = c("../publication_thema/run_AME/")
write_excel =T

scenarioshortnames = substring(wbname,1,2)
#scenarioshortnames = c("S0 Nopol","S1 AME","S2 AME new", paste0("S",4:(length(wbfiles))))
scenarioshortnames = wbname


scenariolabels = c("S0 Nopol" = "Nopol",
                   "S1 AME"= "AME","S2 AME new"="AME new","S3 AMS"="AMS", "S4 AMS new"="AMS new", "S5 AMS1" = "AMS1", 
                   "S6 AMS2" = "AMS2",
                   "S5 AMS1 GJ" = "AMS1 GJ", "S6 AMS2" = "AMS2 GJ")
...
# FICHE RESULTATS CALIBRAGE CODA
...
ggplot(comp_consoCEREN_energie_usage[usage=="Chauffage" & scenario %in% c("CEREN",scenario_ref) & annee %in% 2010:2015]) + 
           geom_bar(aes(scenario,conso_tWhEF, fill = energie), stat="identity") + 
           facet_wrap(~annee, nrow = 1) + 
  theme(axis.text.x = element_text(size = 12, hjust = 1,angle =45))+ ylab("TWh")+
  mytheme_facet_plot + xlab("") +  mytheme_facet_plot + xlab("") + scale_fill_manual("", values = color_energie$color[1:4]) +
   scale_x_discrete(label = c("CEREN", "modèle"))
  
#ggsave("../publication_thema/figures/CompConsoChauffCERREN.pdf",width = 14, height=12)
...
#### tableau export
Surface_parc_Renov_Etat = dcast.data.table(Surface_parc_Renov_Etat[annee %in% c(2010,2016,seq(2020,2050,5))], scenario~annee,value.var = "Surface_Renov")
...
Surface_parc_Renov_Etat_good  <- Surface_parc_Renov_Etat[,lapply(.SD, function(x){round(x/10^3, digits=2)}), .SDcols= as.character(c(2010,2016,seq(2020,2050,5))), by=c("scenario")]
...
Corresp_geste2 = data.table(Geste = unique(cout_geste$GESTE))
Corresp_geste2[Geste  %in% c("FENMOD","FENBBC","GTB"), GESTE_DGEC:="Rénovation faible"]
Corresp_geste2[Geste  %in% c("FEN_MURMOD","ENSMOD"), GESTE_DGEC:="Rénovation moyenne"]
Corresp_geste2[Geste  %in% c("FEN_MURBBC","ENSBBC"), GESTE_DGEC:="Rénovation importante"]
Surfaces_renov_gestes_cum = merge(Surfaces_renov_gestes_cum,Corresp_geste2, by="Geste")


Surfaces_renov_gestes_cum[,sum(surface), by=c("scenario","GESTE_DGEC")]
...
surface_renov_tot[annee%in%2010:2050,sum(surface_renov), by = "scenario"]
surface_renov_tot[annee%in%2010:2030,mean(surface_renov), by = "scenario"]
surface_renov_tot[annee%in%2010:2050,mean(surface_renov), by = "scenario"]
...
ggplot(graph_INVpres) + 
  geom_ribbon(aes(ymin = 0, annee, ymax = INV_Md_cum, group =Type_Inv, fill = Type_Inv)) +
    ...
    ylab("Milliards d'euros") + facet_wrap(~scenario) + 
    scale_x_discrete(breaks = seq(2010,2050,5), labels =
                    seq(2010,2050,5)) +
...
```{r, echo=F, warning=F,}
pander(INV_aides_quinquennat, digits = 2, caption = "Investissements totaux (milliards d'euros) et part des aides entre 2015 et 2022 (approximation du quinquennat)")
```

```{r, echo=F, warning=F,}
pander(INV_aides_2050, digits = 2, caption = "Investissements totaux (milliards d'euros) et part des aides entre 2015 et 2050")
```
...
Em_nationale_energie_type_usage[scenario %in% c("S4 AMS new","S3 AMS", "S5 AMS1", "S6 AMS2"),EmMtCO2_socio := conso_tWhEF*FE_AMS*10^9*10^-12]
Em_nationale_energie_type_usage[scenario %in% c("S4 AMS new","S3 AMS", "S5 AMS1", "S6 AMS2") ==F,EmMtCO2_socio := conso_tWhEF*FE_AME*10^9*10^-12]
Em_nationale_energie_usage[scenario %in% c("S4 AMS new","S3 AMS", "S5 AMS1", "S6 AMS2"),EmMtCO2_socio := conso_tWhEF*FE_AMS*10^9*10^-12]
Em_nationale_energie_usage[scenario %in% c("S4 AMS new","S3 AMS", "S5 AMS1", "S6 AMS2") ==F,EmMtCO2_socio := conso_tWhEF*FE_AME*10^9*10^-12]
