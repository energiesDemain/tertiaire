...
# wbfiles[2]  = "../tertiaire_to_export/resultats_AME_run2/2050_allpol_CEE_CC/Result_csv"
#wbfiles[3] = "../tertiaire/model/Result_csv/run3_AME/2025_allpol_sansCEE"
wbfiles[2] = "../tertiaire/model/Result_csv/run3_AME/2025_sanspol"
# wbfiles[5] = "../tertiaire/model/Result_csv/run3_AME/2025_allpol_sansTrav"
# #wbfiles[4] =═  "../tertiaire/model/Result_csv/2025_allpoll_newcalibbati_subCEE"

####### noms des scenarios ########

wbname = c( "S1 : AME run 3",  
            "S0 : AME run 2",  
            "S2 : AME run 3 sans CEE", 
            "S3 : AME run 3 sans pol", 
            "S4 : AME run 3 sans trav"
            )


### choix du scenario de référence 
scenario_ref =c("S1 : AME run 3")
scenario_toexport = c("S1 : AME run 3")
scenario_sansCEE = c(  "S1 : AME run 3")

wbdir_scenario_toexport = c("../tertiaire_to_export/resultats_AME_run3/2050_allpol/")
...
## Comparaison avec le CEREN 2010-2013
...
tab_PM_energie_all = dcast.data.table(Conso_nationale_energie_branche[annee %in% c("2010","2015","2020","2025","2030","2035","2040","2045","2050"),],scenario + energie + annee ~ usage, fun.aggregate = sum)

tab_PM_energie_all = tab_PM_energie_all[,
                                                list(energie, Chauffage, ECS, Cuisson,
                                                     Elec_spe_hors_clim =
                                                      sum(Auxiliaires,Ventilation,Process,Bureautique,Eclairage,Froid_alimentaire), Climatisation,
                                                     
                                                     PM_Chauffage = Chauffage/sum(Chauffage),
                                                     PM_ECS =ECS/sum(ECS),
 
                                                     PM_Cuisson =Cuisson/sum(Cuisson))
                                                     , by=c("scenario","annee")]

Conso_nationale_usage[annee == "2015", list(usage,conso_tWhEF/11630*10^3)]
...
PM_clim = SURFACE_CLIM[, list(Surface = sum(SURFACES_TOT)),by=c("scenario","annee","Isclim")]
PM_clim =  PM_clim[, list(Surface = sum(Surface),Isclim, PM_clim = Surface/sum(Surface)),by=c("scenario","annee")]
PM_clim_neuf = SURFACE_CLIM[Type_parc_MEDPRO == "N", list(Surface = sum(SURFACES_TOT)),by=c("scenario","annee","Isclim")]
PM_clim_neuf =  PM_clim_neuf[, list(Surface = sum(Surface),Isclim, PM_clim = Surface/sum(Surface)),by=c("scenario","annee")]
...
cout_geste = fread("../docs_completementaires_EN/Bibli_geste_bati_new_CEE_ADEME.csv", 
...
cout_geste[COUT>0, mean(CEE), by="GESTE_DGEC"]
cout_geste[COUT>0, mean(CEE), by="GESTE"]
...
tab_PM_Non_Renov_cum = PM_geste_natio_cum[,list(GESTE_DGEC = "Parc non touché", PM_Geste = 1-sum(PM_Geste)),by=c("annee","scenario","Surface_parc_exi")]
 
tab_PM_Non_Renov_cum[,Surface_Renov_cum := Surface_parc_exi*PM_Geste]
tab_PM_Non_Renov_cum[,Surface_tot_Renov := Surface_parc_exi*(1-PM_Geste)]
 
tab_PM_GTB_cum = Surface_parc_GTB[annee %in% seq(2010,2050,5), list(annee,Surface_tot_Renov=Surface_Renov,Surface_parc_exi,Surface_Renov_cum = cumsum(Surface_Renov)) ,by=c("scenario","GESTE_DGEC")]

tab_PM_GTB_cum[, PM_Geste := Surface_Renov_cum/Surface_parc_exi]
PM_geste_natio_cum = rbind(PM_geste_natio_cum,tab_PM_GTB_cum)
PM_geste_natio_cum =  rbind(PM_geste_natio_cum,tab_PM_Non_Renov_cum[, colnames(PM_geste_natio_cum),with=F])
...
Surface_parc_Renov_Etat_perf[typeRenovationBatiment %in% c("FEN_MURBBC","ENSBBC")]
...
COUTS[typeRenovationBatiment !="Etat initial",list(surface = sum(surface)/10^6,investissement = sum(investissement)/10^6), by=c("scenario","annee","typeRenovationBatiment")]
...
write.table(dcast.data.table(Surfaces_renov_gestes_cum, scenario+Geste ~ annee, value.var = "surface"),"../échanges_BIPE/surfaces_renov_geste.csv",quote = T,dec = ".",sep=";", row.names= F)
write.table(dcast.data.table(Surfaces_renov_gestes_cum, scenario+Geste ~ annee, value.var = "INV"),"../échanges_BIPE/INV_renov_geste.csv",quote = T,dec = ".",sep=";", row.names= F)

write.table(dcast.data.table(Surfaces_renov_syst_cum, scenario+SYSTEME_CHAUD ~ annee, value.var = "surface"),"../échanges_BIPE/surfaces_renov_syst.csv",quote = T,dec = ".",sep=";", row.names= F)
write.table(dcast.data.table(Surfaces_renov_syst_cum, scenario+SYSTEME_CHAUD ~ annee, value.var = "INV"),"../échanges_BIPE/INV_syst_geste.csv",quote = T,dec = ".",sep=";", row.names= F)
...
write.table(Tab_inv_DGEC_all , "../AME_AMS_dataDGEC/chroniques investissements/Inv_type_DGEC.csv",quote = T,row.names = F, sep=";", dec=".")
...
Tab_inv_DGEC_cum = Tab_inv_DGEC_cum[,list(INV_Md = sum(INV)/10^9, surface= sum(surface)) , by=c("scenario","annee","Type_Inv")]

Tab_inv_DGEC_cum = dcast.data.table(Tab_inv_DGEC_cum[annee %in% seq(2010,2050,5)],scenario + Type_Inv~annee,value.var ="INV_Md")
...
writeWorksheet(wb,tab_PM_geste_cum[scenario == scenario_toexport_short], sheet = "sorties_modele_sanstitre", startRow =95)
...
##### forfait CEE par syst
cout_rdt_chauff[,mean(CEE), by="SYSTEME_CHAUD"][order(SYSTEME_CHAUD)]
