...
wbfiles[5]  = "../tertiaire/model/Result_csv/publi_thema/2050_AMS_variante091_surfaces"

####### noms des scenarios ########

wbname = c( paste0("S",1:length(wbfiles)))
  
wbname = c("S0 Nopol","S1 Prix","S2 AME","S3 AMS", "S4 AMS var", paste0("S",5:(length(wbfiles))))

### choix du scenario de référence 

scenario_ref =c("S0 Nopol")
scenario_toexport = c("S4 AMS var")
scenario_sansCEE = c( "S0 Nopol")
scenario_comp_AMS = c("S0 Nopol","S1 Prix","S2 AME","S3 AMS","S4 AMS var")

wbdir_scenario_toexport = c("../AME_AMS_dataDGEC/AMS 2018/variante AMS surfaces/")
write_excel =T

scenarioshortnames = substring(wbname,1,2)
scenarioshortnames = c("S0 Nopol","S1 Prix","S2 AME","S3 AMS","S4 AMS var", paste0("S",5:(length(wbfiles))))
scenarioshortnames = wbname


scenariolabels = c("S0 Nopol" = "S0", "S1 Prix" = "Prix", 
                   "S2 AME"= "AME","S3 AMS"="AMS", "S4 AMS var" = "AMS var")
...
### Parc neuf/ancien
```{r, echo=F, warning=F}
pander(evolbesoinu_usage_type_save[ Usage_MEDPRO2 == "Chauffage",list(scenario),
                                    by=c( "scenario","2015","2020","2025","2030","2035","2050")], 
       digits = 2, caption = "Evolution des besoins unitaires de chauffage du parc existant et du parc neuf (input MEDPRO)")
```
...
conso_u_graph = conso_unitaire_ensemble_usage_EF[usage == "Chauffage" & annee == "2015" & scenario == "S0 Nopol" ,list(scenario,conso_u)]

conso_u_graph = rbind(conso_u_graph,conso_unitaire_usage[annee %in% c("2050") & usage == "Chauffage" & scenario == "S0 Nopol" & Type_parc == "E", list(scenario,conso_u)])

conso_u_graph = rbind(conso_u_graph,conso_unitaire_ensemble_usage_EF[usage == "Chauffage" & annee == "2050",list(scenario,conso_u)])

conso_u_graph[, scenario2 := factor(c("2015","2050 existant Prix constant","2050 Prix constant","2050 prix REF","2050 REF","2050 SIM"), 
                                       levels = c("2015","2050 existant Prix constant","2050 Prix constant","2050 prix REF","2050 REF","2050 SIM"),
                                    labels = c("2015","2050 \n parc existant \n prix constants \n sans pol","2050 \n parc total \n prix constants \n sans pol","2050  \n parc total \n prix croissants \n sans pol","2050  \n parc total \n REF","2050  \n parc total \n SIM"))]

```

```{r Conso_u_Chauff_evol, fig.width=25, fig.height=10, echo=F, warning=F,fig.cap='Conso unitaire 2050', dev=c('png', 'pdf')}
...
#ggsave("../publication_thema/présentation/Conso_u_Chauff_evol.png",width = 24, height=12)
...
```
```{r, echo=F, warning=F,}
pander(dcast.data.table(conso_unitaire_usage[annee %in% c("2009","2010","2015","2020","2030","2035","2050") & usage == "Chauffage"], scenario +Type_parc ~ annee, value.var = "conso_u"), digits = 0, caption = "Consommations unitaires en kWh par m² d'énergie finale pour l'ensemble du parc et pour le chauffage uniquement")
```
...
```{r Conso_u_branche_ini, fig.width=25, fig.height=10, echo=F, warning=F,fig.cap='Conso unitaire initiale', dev=c('png', 'pdf')}
...
```{r Conso_u_branche_2050, fig.width=25, fig.height=10, echo=F, warning=F,fig.cap='Conso unitaire 2050', dev=c('png', 'pdf')}

ggplot(conso_unitaire_branche_type[annee %in% c("2009","2050") & scenario ==scenario_toexport, list(conso_u = sum(conso_tWhEF, na.rm=T)/sum(surface, na.rm=T)*10^9), by=c("annee","usage","nom_branche")],aes(x=annee,conso_u, fill = usage) ) + 
           geom_bar(stat="identity", position = "stack") + 
           facet_wrap(~nom_branche, nrow = 1, label = labeller(nom_branche = label_wrap_gen(10))) +
  theme(axis.text.x = element_text(size = 12, hjust = 1,angle =45))+ ylab("Kwh par m²")+
#  geom_text(aes(label=conso_u),position = position_stack(vjust = 0.5), colour = "white", fontface = "bold", size = 7)+
...
etiq_evol = conso_unitaire_branche_RT[annee %in% c("2009","2050")]
...
ggplot(Em_nationale_energie_usage[usage == "Chauffage"]) +
           geom_area(aes(as.numeric(annee), EmMtCO2_socio, fill = energie)) +
           facet_wrap(~scenario, nrow = 1) +
           mytheme_facet_plot+ theme(legend.position = "top") +
  scale_x_continuous(breaks = seq(2010,2050,10), labels =
                    seq(2010,2050,10)) + scale_fill_manual("",values=color_energie$color) + ylab("MtCO2") +  xlab("Année")+ 
  theme(axis.text.y = element_text(size = 30),strip.text.x = element_text(size=30), legend.text = element_text(size = 20))  
  

 #ggsave("../publication_thema/présentation/Em_chauff_ener.png",width = 24, height=12)
...
#ggsave("../publication_thema/présentation/Em_tot.png",width = 24, height=12)
...
#COnso_an_chauffage = Conso_nationale_energie_type_usage[usage=="Chauffage"]
...
bilan_socio[,Recettes_TC_AME := Emiss_Mt_AME*TC_AME*10^-3]
bilan_socio[,Recettes_TC_AMS := Emiss_Mt_AMS*TC_AMS*10^-3]
...
test =  bilan_socio[scenario %in% c("S2 AME","S3 AMS")]

test[,list(sum(Recettes_TC_AME),sum(Emiss_Mt_AME), sum(conso_tWhEF), sum(Emiss_Mt_AME)/sum(conso_tWhEF)),  by=c("scenario", "energie")]
test[,list(sum(Recettes_TC_AMS),sum(Emiss_Mt_AMS), sum(conso_tWhEF), sum(Emiss_Mt_AMS)/sum(conso_tWhEF)),  by=c("scenario", "energie")]
test[,list(sum(Recettes_TC_AME),sum(Emiss_Mt_AME), sum(conso_tWhEF), sum(Emiss_Mt_AME)/sum(conso_tWhEF)),  by=c("scenario")]
test[,list(sum(Recettes_TC_AMS),sum(Emiss_Mt_AMS), sum(conso_tWhEF), sum(Emiss_Mt_AMS)/sum(conso_tWhEF)),  by=c("scenario")]

test[energie == "Electricité", list(sum(Recettes_TC_AME),sum(Emiss_Mt_AME), 
                                    sum(Recettes_TC_AMS),sum(Emiss_Mt_AMS), sum(conso_tWhEF)),by=c("scenario")]
test[energie == "Gaz", list(sum(Recettes_TC_AME),sum(Emiss_Mt_AME), 
                                    sum(Recettes_TC_AMS),sum(Emiss_Mt_AMS), sum(conso_tWhEF)),by=c("scenario")]
...
bilan_socio[annee=="2050",Recettes_taxes_ener_AME_actu := Recettes_taxes_ener_AME*coefactu*10]
bilan_socio[annee=="2050",Recettes_taxes_ener_AMS_actu := Recettes_taxes_ener_AMS*coefactu*10]
bilan_socio[annee=="2050",Recettes_TC_AME_actu := Recettes_TC_AME*coefactu*10]
bilan_socio[annee=="2050",Recettes_TC_AMS_actu := Recettes_TC_AMS*coefactu*10]
bilan_socio[annee=="2050",Subvention_elec_actu := sub_elec*coefactu*10]
...
table_results_socio = data.table(scenario = c("REF","SIM","REF prix SIM","SIM prix REF")) 

## facture
table_results_socio[scenario == "REF",Fact :=bilan_socio_cum2[scenario == "S2 AME",Depense_socio_AME_actu] ]
table_results_socio[scenario == "SIM",Fact :=bilan_socio_cum2[scenario == "S3 AMS",Depense_socio_AMS_actu] ]
table_results_socio[scenario == "REF prix SIM",Fact :=bilan_socio_cum2[scenario == "S2 AME",Depense_socio_AMS_actu] ]
table_results_socio[scenario == "SIM prix REF",Fact :=bilan_socio_cum2[scenario == "S3 AMS",Depense_socio_AME_actu] ]

## INV
table_results_socio[scenario == "REF",INV :=bilan_socio_cum2[scenario == "S2 AME", INV_Md_actu] ]
table_results_socio[scenario == "SIM",INV :=bilan_socio_cum2[scenario == "S3 AMS",INV_Md_actu] ]
table_results_socio[scenario == "REF prix SIM",INV :=bilan_socio_cum2[scenario == "S2 AME",INV_Md_actu] ]
table_results_socio[scenario == "SIM prix REF",INV :=bilan_socio_cum2[scenario == "S3 AMS",INV_Md_actu] ]

## EMissions
table_results_socio[scenario == "REF",Em :=bilan_socio_cum2[scenario == "S2 AME",Emiss_Mt_AME_actu] ]
table_results_socio[scenario == "SIM",Em :=bilan_socio_cum2[scenario == "S3 AMS",Emiss_Mt_AMS_actu] ]
table_results_socio[scenario == "REF prix SIM",Em :=bilan_socio_cum2[scenario == "S2 AME",Emiss_Mt_AMS_actu] ]
table_results_socio[scenario == "SIM prix REF",Em :=bilan_socio_cum2[scenario == "S3 AMS",Emiss_Mt_AME_actu] ]

table_results_socio[,CoutTot := Fact + INV]

## coût tonne
table_results_socio[,CoutTonne :=(CoutTot - table_results_socio[scenario == "REF",CoutTot])/-(Em - table_results_socio[scenario == "REF",Em])*10^3, by="scenario"]

#### recettes taxes
table_results_socio[scenario == "REF",TC :=bilan_socio_cum2[scenario == "S2 AME",Recettes_TC_AME_actu] ]
table_results_socio[scenario == "SIM",TC :=bilan_socio_cum2[scenario == "S3 AMS",Recettes_TC_AMS_actu] ]
table_results_socio[scenario == "REF prix SIM",TC :=bilan_socio_cum2[scenario == "S2 AME",Recettes_TC_AMS_actu] ]
table_results_socio[scenario == "SIM prix REF",TC :=bilan_socio_cum2[scenario == "S3 AMS",Recettes_TC_AME_actu] ]

table_results_socio[scenario == "REF",Taxes_ener :=bilan_socio_cum2[scenario == "S2 AME",Recettes_taxes_ener_AME_actu] ]
table_results_socio[scenario == "SIM",Taxes_ener :=bilan_socio_cum2[scenario == "S3 AMS",Recettes_taxes_ener_AMS_actu] ]
table_results_socio[scenario == "REF prix SIM",Taxes_ener :=bilan_socio_cum2[scenario == "S2 AME",Recettes_taxes_ener_AMS_actu] ]
table_results_socio[scenario == "SIM prix REF",Taxes_ener :=bilan_socio_cum2[scenario == "S3 AMS",Recettes_taxes_ener_AME_actu] ]

table_results_socio[scenario == "SIM",sub_elec :=bilan_socio_cum2[scenario == "S3 AMS",Subvention_elec_actu] ]
table_results_socio[scenario == "REF",sub_elec :=bilan_socio_cum2[scenario == "S2 AME",Subvention_elec_actu] ]
table_results_socio[scenario == "REF prix SIM",sub_elec :=bilan_socio_cum2[scenario == "S3 AMS",Subvention_elec_actu] ]
table_results_socio[scenario == "SIM prix REF",sub_elec :=bilan_socio_cum2[scenario == "S2 AME",Subvention_elec_actu] ]
...
table_results_socio[,CoutTonne_COFP :=(CoutTot+ COFP - table_results_socio[scenario== "REF",CoutTot+ COFP])/-(Em - table_results_socio[scenario== "REF",Em])*10^3, by="scenario"]
...
table_results_usager = data.table(scenario = c("REF","SIM","REF prix SIM","SIM prix REF")) 

## facture
table_results_usager[scenario == "REF",Fact :=bilan_usager_cum2[scenario == "S2 AME",Depense_usager_AME_actu] ]
table_results_usager[scenario == "SIM",Fact :=bilan_usager_cum2[scenario == "S3 AMS",Depense_usager_AMS_actu] ]
table_results_usager[scenario == "REF prix SIM",Fact :=bilan_usager_cum2[scenario == "S2 AME",Depense_usager_AMS_actu] ]
table_results_usager[scenario == "SIM prix REF",Fact :=bilan_usager_cum2[scenario == "S3 AMS",Depense_usager_AME_actu] ]

## INV
table_results_usager[scenario == "REF",INV :=bilan_usager_cum2[scenario == "S2 AME", INV_Md_actu] ]
table_results_usager[scenario == "SIM",INV :=bilan_usager_cum2[scenario == "S3 AMS",INV_Md_actu] ]
table_results_usager[scenario == "REF prix SIM",INV :=bilan_usager_cum2[scenario == "S2 AME",INV_Md_actu] ]
table_results_usager[scenario == "SIM prix REF",INV :=bilan_usager_cum2[scenario == "S3 AMS",INV_Md_actu] ]

## EMissions
table_results_usager[scenario == "REF",Em :=bilan_usager_cum2[scenario == "S2 AME",Emiss_Mt_AME_actu] ]
table_results_usager[scenario == "SIM",Em :=bilan_usager_cum2[scenario == "S3 AMS",Emiss_Mt_AMS_actu] ]
table_results_usager[scenario == "REF prix SIM",Em :=bilan_usager_cum2[scenario == "S2 AME",Emiss_Mt_AMS_actu] ]
table_results_usager[scenario == "SIM prix REF",Em :=bilan_usager_cum2[scenario == "S3 AMS",Emiss_Mt_AME_actu] ]
...
write.table(table_results_socio[, list(scenario,Fact,INV,COFP,CoutTot_COFP,Em,CoutTonne_COFP, CoutTot, CoutTonne)],
            file = "../publication_thema/présentation/bilan_socio_AMSvar.csv",quote = T, sep=";",row.names = F)
write.table(table_results_socio[, list(scenario, TC,Taxes_ener,sub_elec,COFP)],
            file = "../publication_thema/présentation/bilan_Etat_AMSvar.csv",quote = T, sep=";",row.names = F)

write.table(table_results_usager[, list(scenario, Fact,INV,CoutTot)],
            file = "../publication_thema/présentation/bilan_usager_AMSvar.csv",quote = T, sep=";",row.names = F)


```

```{r Evol_Recettes_taxes, fig.width=25, fig.height=10, echo=F, warning=F,fig.cap='Evolution recettes de la CC', dev=c('png', 'pdf')}
ggplot() + geom_line(data = graph_taxes, aes(annee,TC*10^3, group = scenario, color= scenario), size = 4) + 
  theme(axis.text.y = element_text(size = 30))+ scale_x_discrete(breaks =c(seq(2010,2050,10))) + ylab("Millions d'euros") + 
  mytheme_facet_plot + xlab("")

#ggsave("../publication_thema/présentation/EvolRecettesCC.png",width = 24, height=12)
...
wb <- loadWorkbook(paste0(wbdir_scenario_toexport,"Sorties_AME_AMS_2018.xlsx"), create = F)
...
parc_melted[, list(Surface = sum( Surface)), by=c("scenario","Branche_MEDPRO","annee")]
