...
public abstract class SegmentConso {
	private String id;
	...
	public void setIdAgregEcs(String idagregecs) {
		this.id = newIdAgregEcs(getIdagreg(), getAnneeRenovSys(), getTypeRenovSys());
	}

	public String getIdAgregEcs() {
		return newIdAgregEcs(getId(), getAnneeRenovSys(), getTypeRenovSys());
	}

	public void setIdAgregClim(String idagregecs) {
		this.id = newIdAgregEcs(getIdagreg(), getAnneeRenovSys(), getTypeRenovSys());
	}

	public String getIdAgregClim() {
		return newIdAgregEcs(getId(), getAnneeRenovSys(), getTypeRenovSys());
	}
	...
	protected String newIdAgregEcs(String idAgreg, String anneeRenovSys, TypeRenovSysteme typeRenovSys) {
		// TODO EDR : à valider avec EMF
		StringBuffer bufferIdAgreg = new StringBuffer();
		bufferIdAgreg.append(idAgreg);
		bufferIdAgreg.append(anneeRenovSys);
		bufferIdAgreg.append(typeRenovSys);

		return bufferIdAgreg.toString();
	}
}